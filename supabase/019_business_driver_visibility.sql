-- Let a business owner see the assigned driver's public trip profile, not private documents.
  ...
  select
    ...
    or exists (
      ...
      where business.owner_id = auth.uid()
        and driver.user_id = p_profile_id
        and delivery.status in ('accepted','picking_up','picked_up','delivering','delivered','cancelled')
    );
$$;
...
using (
  exists (
    ...
    from public.business_delivery_requests delivery
    join public.business_accounts business on business.id = delivery.business_id
    where delivery.accepted_driver_id = drivers.id
